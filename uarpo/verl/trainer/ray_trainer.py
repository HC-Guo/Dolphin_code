...
class Role(IntEnum):
    ...
    Actor = auto()              # 策略网络
    Rollout = auto()            # 采样器
    ActorRollout = auto()       # 混合角色
    Critic = auto()             # 价值网络
    RefPolicy = auto()          # 参考策略
    RewardModel = auto()        # 奖励模型
    ActorRolloutRef = auto()    # 混合角色（参考策略+采样器）
...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0):
    ...
    elif adv_estimator == AdvantageEstimator.UARPO:
        advantages, returns = core_algos.compute_uarpo_outcome_advantage(token_level_rewards, response_mask, index)
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if (
            config.algorithm.adv_estimator in (AdvantageEstimator.UARPO, AdvantageEstimator.RLOO)
            and config.worker.rollout.n == 1
        ):
            raise ValueError("UARPO and RLOO algorithm need `config.worker.rollout.n > 1`.")
        ...
        for _ in tqdm(range(self.config.trainer.total_epochs), desc="Epoch", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                with timer("step", timing_raw):
                    ...
                    # Please take care when you implement group based adv computation such as UARPO and rloo
